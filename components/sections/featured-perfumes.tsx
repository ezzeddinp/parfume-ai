...
export default function FeaturedPerfumes() {
  ...
  useEffect(() => {
    const fetchPerfumes = async () => {
      const { data, error } = await supabase.from("parfume-product").select("*").order("id")
      ...
      const parsed = data.map((p: any) => ({
        ...
        brand: p.brand,
        price: p.price,
        image: p.image || `https://source.unsplash.com/featured/?perfume&sig=${p.id}`,
        description: p.description || "No description",
        originalPrice: p.originalPrice || "",
        rating: p.rating || 4.5,
        reviews: p.reviews || Math.floor(Math.random() * 300 + 100),
  ...
  return (
    <section ref={perfumesRef} className="py-32 px-4 relative z-10">
      <div className="container mx-auto">
        ...
        >
          <motion.h3
            className="text-6xl font-black mb-8 bg-gradient-to-r from-white to-blue-400 bg-clip-text text-transparent"
            animate={perfumesInView ? { opacity: 1, y: 0 } : {}}
          >
            FEATURED FRAGRANCES
          </motion.h3>
        ...
        <div className="grid md:grid-cols-3 gap-8 lg:gap-12">
          {perfumes.map((perfume, index) => (
            ...
            >
              <div className="bg-gradient-to-b from-gray-900/90 to-black/90 backdrop-blur-xl border border-gray-700/50 rounded-3xl overflow-hidden hover:border-blue-500/50 transition-all duration-500 flex flex-col h-full">
